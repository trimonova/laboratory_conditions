...
plt.contour(X_matrix, Y_matrix, Func_matrix_remake_in_Time[60], levels=0)
...
plt.plot(viscosity_in_Time[0][:,10])
plt.plot(viscosity_in_Time[60][:,10])
plt.plot(viscosity_in_Time[-1][:,10])
plt.title('viscosity')
plt.show()
...
ax1.plot(np.gradient(Pres_distrib_in_Time[0], delta_r_list[0], delta_fi_list[0])[0][:,10])
ax1.plot(np.gradient(Pres_distrib_in_Time[60], delta_r_list[0], delta_fi_list[0])[0][:,10])
ax1.plot(np.gradient(Pres_distrib_in_Time[-1], delta_r_list[0], delta_fi_list[0])[0][:,10])

ax1.tick_params(axis='y', labelcolor='red')

ax2 = ax1.twinx()

ax2.plot(viscosity_in_Time[0][:,10])
ax2.plot(viscosity_in_Time[60][:,10])
ax2.plot(viscosity_in_Time[-1][:,10])
plt.show()

filename = "120_step"
fields = ["press_gradient", "viscosity", "velosity"]
...
with open(filename, 'w') as csvfile:
    ...
    columns = [np.gradient(Pres_distrib_in_Time[119], delta_r_list[0], delta_fi_list[0])[0][0:40,10], viscosity_in_Time[119][0:40, 10], velocity_in_Time[119][0:40, 10]]
    rows = zip(*columns)
