...
coord_matrix_rad_cell = []
coord_matrix_cart_cell = []
for n in range(len(delta_r_list)):
    coord_line_rad_cell = []
    coord_line_cart_cell = []
    #r = sum(delta_r_list[0:n]) + r_well
    r = delta_r * n + r_well
    #r = delta_r * n + r_well
    for m in range(len(delta_fi_list)):
        ...
        coord_line_rad_cell.append((r, fi))
        coord_line_cart_cell.append((r * np.cos(fi), r * np.sin(fi)))
        # coord_matrix_rad[n][m] = (r, fi)
        # coord_matrix_cart[n][m] = (r*np.cos(fi), r*np.sin(fi))
    coord_matrix_rad_cell.append(coord_line_rad_cell)
    coord_matrix_cart_cell.append(coord_line_cart_cell)

delta_t = 0.5
Pres = 0 * 10 ** 5
#Q_center = 0.2 * 10 ** (-8)  # из лаб. данных - 0.2*10**(-6) m3/s
Q_center = 0.2 * 10 ** (-6)
s = 0.01*0.01 # площадь поверхности затравки
q = Q_center/s/4
#q =Q_center
...
X_matrix_cell = np.zeros((N_r_full, M_fi_full))
Y_matrix_cell = np.zeros((N_r_full, M_fi_full))
for n in range(N_r_full):
    #r = sum(delta_r_list[0:n]) + r_well
    r = delta_r * n + r_well
    for m in range(M_fi_full):
        ...
        X_matrix_cell[n][m] = r * np.cos(fi)
        Y_matrix_cell[n][m] = r * np.sin(fi)
