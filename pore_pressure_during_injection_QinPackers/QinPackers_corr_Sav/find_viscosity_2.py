...
def find_viscosity(mu_oil, mu_water, func_matrix, coord_matrix, delta_r_list, delta_fi_list, N_r_full, M_fi_full):
    viscosity_matrix_1 = np.zeros((N_r_full, M_fi_full))
        ...
        for n in range(N_r_full-1):
            ...
            if func_matrix_cell[0] > 0  and func_matrix_cell[1] > 0 and func_matrix_cell[2] > 0 and func_matrix_cell[3] > 0:
                ...
                if len(set(oil_area_coords)) > 2:
                    ...
                    viscosity_matrix_1[n][m] = oil_area/cell_area * mu_oil + (cell_area-oil_area)/cell_area * mu_water
                    #print(n, m, 'oil area-', oil_area, 'cell area-', cell_area)

                    if viscosity_matrix_1[n][m] > 0.2 or viscosity_matrix_1[n][m] < 0.002:
                        print(coord_matrix_cell, cell_area)
                        print(oil_area_coords, oil_area)
                        print(func_matrix_cell)
                        print(viscosity_matrix_1[n][m], n, m, func_matrix_cell)
                        raise ValueError
                else:
                    viscosity_matrix_1[n][m] = mu_water
            # if n == 50 and m == 4:
    ...
    for m in range(M_fi_full):
        viscosity_matrix_1[N_r_full-1][m] = mu_water

    viscosity_matrix_right = np.zeros((N_r_full, M_fi_full))
    for m in range(M_fi_full):
        for n in range(N_r_full-1):
            oil_area_coords = []  # координаты области с маслом в одной ячейке
            ...
            if m != 0:
                ...
                func_matrix_cell = [func_matrix[n][m], func_matrix[n + 1][m], func_matrix[n + 1][M_fi_full-1],
                                    func_matrix[n][M_fi_full-1], func_matrix[n][m]]
                coord_matrix_cell = [(coord_matrix[n][m][0], 2*np.pi), (coord_matrix[n + 1][m][0], 2*np.pi), coord_matrix[n + 1][M_fi_full-1],
                                     coord_matrix[n][M_fi_full-1], coord_matrix[n][m]]
            ...
            if func_matrix_cell[0] > 0  and func_matrix_cell[1] > 0 and func_matrix_cell[2] > 0 and func_matrix_cell[3] > 0:
                viscosity_matrix_right[n][m] = mu_oil
            elif func_matrix_cell[0] < 0  and func_matrix_cell[1] < 0 and func_matrix_cell[2] < 0 and func_matrix_cell[3] < 0:
                viscosity_matrix_right[n][m] = mu_water
            else:
                rad = sum(delta_r_list[0:n+1])
                delta = rad/np.cos(delta_fi_list[m]) - rad
                min_coef_proport = delta/(delta_r_list[n] - delta)
                ...
                if len(set(oil_area_coords)) > 2:
                    ...
                    # pgon_cell = Polygon(zip(x_cell_area_coords, y_cell_area_coords))  # Assuming the OP's x,y coordinates
                    # cell_area = pgon_cell.area
                    viscosity_matrix_right[n][m] = oil_area/cell_area * mu_oil + (cell_area-oil_area)/cell_area * mu_water
                    #print(n, m, 'oil area-', oil_area, 'cell area-', cell_area)

                    if viscosity_matrix_right[n][m] > 0.2 or viscosity_matrix_right[n][m] < 0.002:
                        print(coord_matrix_cell, cell_area)
                        print(oil_area_coords, oil_area)
                        print(func_matrix_cell)
                        print(viscosity_matrix_right[n][m], n, m, func_matrix_cell)
                        raise ValueError
                else:
                    viscosity_matrix_right[n][m] = mu_water
            # if n == 50 and m == 4:
            ...
            #     print('cell', x_cell_area_coords, y_cell_area_coords)
    for m in range(M_fi_full):
        viscosity_matrix_right[N_r_full-1][m] = mu_water
