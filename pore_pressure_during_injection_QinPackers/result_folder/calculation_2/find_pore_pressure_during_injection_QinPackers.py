...
Func_matrix_remake, func_list_0, func_list_end = replace_boundary((M_1+0.5)*delta_fi_list[0], (M_2+0.5)*delta_fi_list[0], 0.005, frac_length_1, frac_length_2, M_fi_full, N_r_full, coord_matrix_cart_cell, r_well, delta_r, delta_fi_list)
...
for n in range(N_1):
    plt.scatter((n*delta_r_list[0]+r_well+delta_r/2)*np.cos((M_1+0.5)*delta_fi_list[0]), (n*delta_r_list[0]+r_well+delta_r/2)*np.sin((M_1+0.5)*delta_fi_list[0]))
for n in range(N_2):
...
for t in range(T_exp):
    ...
    # plt.plot(Pres_distrib[:, 47])
    # plt.plot(Pres_distrib[:, 44])
    # #plt.plot(Pres_distrib[:, 42])
    # plt.show()

    Func_matrix, velocity, v_r, v_fi, grad_fi_distrib, velocity_distrib = define_func_matrix(Pres_distrib, Func_matrix_remake, perm, delta_r_list, delta_fi_list,
                                               delta_t, r_well, q, viscosity_matrix,  N_r_full, M_fi_full, N_1, N_2, M_1, M_2, func_list_0, func_list_end)
    ...
    # plt.show()
    #
    # plt.plot(velocity[10, :])
    # plt.plot(velocity[15, :])
    # #plt.plot(velocity[:, 47])
    # #plt.plot(velocity[:, 44])
    # # plt.plot(Pres_distrib[:, 42])
    # plt.show()
    ...
    # plt.show()
    #
    # surf = plt.contourf(X_matrix_cell, Y_matrix_cell, Func_matrix, cmap=plt.get_cmap('jet'))
    # plt.title('Func_matrix')
    # plt.colorbar(surf)
    # plt.show()

    bound_coords_rad_new, bound_coords_cart_new = find_bound_coords(Func_matrix, coord_matrix_rad_cell, delta_r_list,
                                                                    delta_fi_list, M_fi_full, N_r_full)

    Func_matrix_remake = find_func_matrix_remake(coord_matrix_cart_cell, M_fi_full, N_r_full, bound_coords_cart_new)
