...
def PorePressure_in_Time(N_r_full, M_fi_full, Pres_distrib, c3_oil, c3_water, CP_dict, q, wells_frac_coords, wells_coords,
    ...
    for m in range(M_fi_full):
        ...
        for n in range(0, N_r_full):
            r_i = n*delta_r_list[n] + delta_r_list[n] + r_well
            r_i_minus_0_5 = r_i - delta_r_list[n]/2
            r_i_plus_0_5 = r_i + delta_r_list[n] / 2
            k_fluid = viscosity_matrix[n][m] * fi * C_total / perm
            c3_fluid = k_fluid/delta_t

            if n == 0:
                A[n][n] = -r_i_plus_0_5 / r_i / delta_r_list[n] ** 2 - 2 / r_i ** 2 / delta_fi_list[m] ** 2 - c3_fluid
                A[n][n+1] = r_i_plus_0_5 / r_i / delta_r_list[n] ** 2
            elif n == N_r_full-1:
                A[n][n] = -r_i_minus_0_5 / r_i / delta_r_list[n] ** 2 - 2 / r_i ** 2 / delta_fi_list[m] ** 2 - c3_fluid
                A[n][n - 1] = r_i_minus_0_5 / r_i / delta_r_list[n] ** 2
            else:

                A[n][n - 1] = r_i_minus_0_5/r_i/delta_r_list[n]**2

                #print(n, len(delta_r_list), len(delta_fi_list))
                A[n][n] = - c3_fluid - 2/delta_r_list[n]**2 - 2/r_i**2/delta_fi_list[m]**2

                A[n][n + 1] = r_i_plus_0_5/r_i/delta_r_list[n]**2


            c4 = 1 / delta_fi_list[m] ** 2
            A_sym_right[n][n] = c4 / (r_i) ** 2
            A_sym_left[n][n] = c4 / (r_i) ** 2
    ...
    for m in range(M_fi_full):
        for n in range(N_r_full):
            ...
            if n == 0:
                ...
                r_i_minus_0_5 = r_i - delta_r_list[n] / 2
                B[j][0] = B[j][0] - r_i_minus_0_5/r_i/delta_r_list[0]*(viscosity_matrix[n][m]*q/perm)
            j += 1
