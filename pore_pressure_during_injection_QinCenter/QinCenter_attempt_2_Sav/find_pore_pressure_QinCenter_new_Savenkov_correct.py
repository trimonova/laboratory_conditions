...
def PorePressure_in_Time(N_r_full, M_fi_full, Pres_distrib, c3_oil, c3_water, CP_dict, q, wells_frac_coords, wells_coords,
    ...
    for m in range(M_fi_full):
        ...
        for n in range(0, N_r_full):
            ...
            if n != N_r_full-1:
                ...
                mu_i_plus_0_5 = 2*(1/viscosity_matrix[n][m] + 1/viscosity_matrix[n + 1][m])**(-1)
            if n != 0:
                mu_i_minus_0_5 = (viscosity_matrix[n][m] + viscosity_matrix[n - 1][m]) / 2
                mu_i_minus_0_5 = 2 * (1 / viscosity_matrix[n][m] + 1 / viscosity_matrix[n - 1][m]) ** (-1)
            ...
            if m == M_fi_full-1:
                ...
                mu_j_plus_0_5 = 2 * (1 / viscosity_matrix[n][m] + 1 / viscosity_matrix[n][0]) ** (-1)
                mu_j_minus_0_5 = 2 * (1 / viscosity_matrix[n][m] + 1 / viscosity_matrix[n][m-1]) ** (-1)
            else:
                mu_j_plus_0_5 = (viscosity_matrix[n][m] + viscosity_matrix[n][m+1]) / 2
                mu_j_minus_0_5 = (viscosity_matrix[n][m] + viscosity_matrix[n][m-1]) / 2
                mu_j_plus_0_5 = 2 * (1 / viscosity_matrix[n][m] + 1 / viscosity_matrix[n][m+1]) ** (-1)
                mu_j_minus_0_5 = 2 * (1 / viscosity_matrix[n][m] + 1 / viscosity_matrix[n][m-1]) ** (-1)
            #k_fluid = fi * C_total
            #c3_fluid = k_fluid/delta_t
    ...
    for coord_couple in wells_coords_reverse:
        ...
        for cell_number in range(len(A_well_column)):
            if A_well_column[cell_number] != 0:
                print(cell_number, B[cell_number][0], A_well_column[cell_number], CP_dict[coord_couple], B[cell_number][0] - A_well_column[cell_number]*CP_dict[coord_couple])
                print()
                B[cell_number][0] = B[cell_number][0] - A_well_column[cell_number]*CP_dict[coord_couple]
                if B[cell_number][0] < -4:
                    print(B[cell_number][0])
                    print(B[cell_number][0])


    #A_full_array = A_full.toarray()
    ...
    print("type_A_full_Sav", type(A_full))
